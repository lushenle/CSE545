...
int main(int argc, char *argv[])
{
  ...
  int buffer_length = strlen(getenv("BASE")) + 128;
  path = malloc(buffer_length);
  sprintf(path, "%s/flag", getenv("BASE"));

  // damn it - this is very important
  real_path = malloc(buffer_length);
  ...
  // Wait a little bit to ensure that the file is really there
  // I heard that magnetic failure is a real thing...
  sleep(5);
