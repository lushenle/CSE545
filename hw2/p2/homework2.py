...
for N in range(2, 10):
    ...
    for i in range(0, len(secretContentHex)):
        ...
        filenameHexNum = filenameHex[
            (2 * i) % (len(filenameHex)) : (2 * i) % (len(filenameHex)) + 2
        ]
        filenameHexInt = lib.binary_conversion(16, 10, filenameHexNum)
        afterShift = shift(filenameHexInt, N)  # 循环左移N位
        if len(afterShift) < 8:
            afterShift = "0" * (8 - len(afterShift)) + afterShift
        afterShiftInt = lib.binary_conversion(2, 10, afterShift)
        keyInt = secretNum ^ afterShiftInt
        afterShiftHex = lib.binary_conversion(10, 16, keyInt)
        try:
...
for i in range(0, len(secretContentHex)):
    ...
    key_byteHexNum = lib.binary_conversion(16, 10, key_byteHexNum)
